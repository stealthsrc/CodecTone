...
public sealed class FfprobeService(string ffprobePath)
{
    public async Task<ProbeInfo> ProbeAsync(string path, CancellationToken cancellationToken = default)
    {
        ...
        var arguments = new[]
        {
            ...
            "format=duration,size,bit_rate:format_tags:stream=codec_type,codec_name,sample_rate,channels,bits_per_sample,bits_per_raw_sample,bit_rate:stream_disposition=attached_pic",
            "-of", "json", path,
        };
